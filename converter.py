...
for i in range(1, nrecords):
    ...
    if radar_type[i-1] == 'iskef':
        ...
        H1_iskef.append(float(records[i][3]))
        H2_iskef.append(float(records[i][5]))
    elif radar_type[i-1] == 'isx1':
        nrecords_isx1 += 1
        time_isx1.append(val_time[i-1][1])
        H1_isx1.append(float(records[i][4]))
        H2_isx1.append(float(records[i][6]))
    elif radar_type[i-1] == 'isx2':
        nrecords_isx2 += 1
        time_isx2.append(val_time[i-1][1])
        H1_isx2.append(float(records[i][4]))
        H2_isx2.append(float(records[i][6]))
    else:
        nrecords_isegs += 1
        time_isegs.append(val_time[i-1][1])
        H1_isegs.append(float(records[i][3]))
        H2_isegs.append(float(records[i][5]))
...
for j in range(0, max(nrecords_iskef,nrecords_isx1,nrecords_isx2,nrecords_isegs)):
    ...
    if j <= nrecords_isx1 and nrecords_isx1 != 0:
        ...
        hour_isx1.append(time_iskef[j][0:2])
        minute_isx1.append(time_iskef[j][3:5])
    if j <= nrecords_isx2 and nrecords_isx2 != 0:
        H_avg_isx2.append(0.5 * (H1_isx2[j] + H2_isx2[j]))
        hour_isx2.append(time_iskef[j][0:2])
        minute_isx2.append(time_iskef[j][3:5])
    if j <= nrecords_isegs and nrecords_isegs != 0:
        H_avg_isegs.append(0.5 * (H1_isegs[j] + H2_isegs[j]))
        hour_isegs.append(time_iskef[j][0:2])
        minute_isegs.append(time_iskef[j][3:5])
